...
export function checkFrogCollsion(frog:Frog, obstacle:Car|Insect|Power|Bomb){
    return (obstacle.x < frog.x + frog.width &&
      obstacle.x + obstacle.width > frog.x &&
      obstacle.y < frog.y + frog.height &&
      obstacle.y + obstacle.height > frog.y)
}

export function removeFromArray(array:(Frog|Car|Bomb|Power|Insect|Coin)[],element:Frog|Car|Bomb|Power|Insect|Coin):void{
